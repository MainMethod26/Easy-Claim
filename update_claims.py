...
with open('backend/src/endpoints/claims.ts', 'r') as f:
    content = f.read()

# Let's just rewrite claims.ts completely to match the document while keeping the existing endpoints for backward compatibility.
new_content = """import { Hono } from 'hono'
...
with open('backend/src/endpoints/claims.ts', 'w') as f:
    f.write(new_content)
