...
def main() -> int:
    ...
    for s in signals:
        ...
        sql.append(
            ...
            f"interpretation, execution, feature_snapshot, computed_at) VALUES ({sql_str(s.claim_id)}, {sql_str(MODEL_VERSION)}, "
            f"{s.classical_anomaly}, {s.quantum_anomaly}, {sql_str(s.interpretation)}, 'simulator', "
            f"{sql_str(json.dumps(s.features, separators=(',', ':')))}, {sql_str(computed_at)});"
        )
    ...
    # an ASSESSOR of ins_discovery can call POST /screen on them. Their signals are the scores of the
    # synthetic rows they copy. This file is for the LOCAL demo database only.
    ...
    demo = [
        ...
        "-- (tenant ins_discovery), stage Verified, plus their SIMULATOR screening signals.",
    ...
    for demo_id, src in (("claim_demo_normal", normal_pick), ("claim_demo_unusual", high_pick)):
        row = next(r for r in dev_rows if r["id"] == src.claim_id)
        demo.append(
            ...
            f"'{demo_id}', 'user123', 'pol_disc_001', 'ins_discovery', 'Verified', 'Pending', {sql_str(row['category'])}, "
            f"{sql_str(row['cause_of_loss'])}, {sql_str(row['incident_date'])}, {sql_str(row['created_at'])}, {sql_str(row['updated_at'])});"
        )
        demo.append(
            "INSERT OR REPLACE INTO screening_signals (claim_id, model_version, classical_anomaly, quantum_anomaly, interpretation, execution, feature_snapshot, computed_at) VALUES ("
            f"'{demo_id}', {sql_str(MODEL_VERSION)}, {src.classical_anomaly}, {src.quantum_anomaly}, {sql_str(src.interpretation)}, 'simulator', "
            f"{sql_str(json.dumps({**src.features, '_source': src.claim_id, '_synthetic': True}, separators=(',', ':')))}, {sql_str(computed_at)});"
        )
