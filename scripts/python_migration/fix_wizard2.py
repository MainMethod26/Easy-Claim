...
# 2. Fix document upload modal
pattern2 = r"showModalBottomSheet\(\s*context: context,\s*isScrollControlled: true,\s*backgroundColor: Colors\.white,\s*shape: const RoundedRectangleBorder\(\s*borderRadius: BorderRadius\.vertical\(top: Radius\.circular\(24\)\),\s*\),\s*builder: \(sheetContext\) => (SafeArea\([^;]+)\);"
def repl2(match):
    body = match.group(1)
