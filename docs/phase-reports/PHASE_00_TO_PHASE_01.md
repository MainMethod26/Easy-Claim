...
Date: 2026-09-26
Branch: `cyber`
Phase 0 record: commits `c73f3e1` (implementation) and `1468897` (HEAD at the Phase 1 precheck), on top of base `main` `efb6cc3`.
Phase 1 state described here: the **uncommitted working tree** on `cyber` at the time of writing (19 files modified, 7 files/directories added; see the appendix). Where this report and git HEAD disagree, the working tree is what was read.

Status vocabulary: IMPLEMENTED / PARTIAL / NOT IMPLEMENTED / PLANNED / BLOCKED / NOT TESTED / DECISION REQUIRED / OPEN.

How this report was produced: every claim below was checked against the working tree (`backend/src`, `backend/migrations`, `backend/test`, `backend/scripts`, `backend/wrangler.toml`) and against the HEAD versions of the changed files (`git show HEAD:<path>`). `npx vitest run` was executed once in `backend/`: **12 files, 175 passed, 1 todo**. `npm run typecheck` and `npm audit` were not re-run for this report (their Phase 0 results are in `PHASE_01_PRECHECK.md` section 1).

The Phase 0 documents (`docs/security/README.md`, `docs/BACKEND_INVENTORY.md`, `docs/security/BACKEND_SECURITY_HANDOFF.md`, `docs/security/SECURITY_IMPLEMENTATION_PLAN.md` and the rest of `docs/security/`) are the Phase 0 record. This report reads them; it does not rewrite them. Where they are now out of date because Phase 1 changed the code, section 6 says so and points to the Phase 1 documentation update.

---

## 1. What Phase 0 delivered

Phase 0 (branch `cyber`, commit `c73f3e1`) turned a stub API with one control (parameterised SQL in `PolicyModel.getMyCovers`, `backend/src/models/policyModel.ts`) into an API with authorization, validation and audit, but **without authentication**. Its own summary is `docs/security/README.md` (posture table) and `docs/BACKEND_INVENTORY.md` section 2 (before/after table).

| Area | Phase 0 status (as recorded) | Phase 0 mechanism |
|---|---|---|
| Authentication | NOT IMPLEMENTED (dev stub only) | `backend/src/security/actor.ts` `resolveActor()` trusted `X-Dev-Actor-Id` / `X-Dev-Actor-Role` only when `ALLOW_DEV_ACTOR_HEADERS === 'true'`; otherwise every `/api/v1/*` request got 401 |
| Function-level authorization | IMPLEMENTED | `backend/src/security/rbac.ts` `requireRole(...roles)` with audit `authz.role_denied` |
| Object-level authorization | IMPLEMENTED (owner or *any* insurer) | `backend/src/security/claimAccess.ts` `loadAuthorizedClaim(c, id, 'read' \| 'owner-write')`; deny and missing both 404 with audit `authz.claim_access_denied` `{mode, exists}` |
| Tenant scoping | NOT IMPLEMENTED | no tenant column, no `tenantId` on `Actor` |
| Property-level authorization | IMPLEMENTED | `backend/src/security/validation.ts` strict zod schemas + `validate()` |
| Claim state machine | IMPLEMENTED (customer edges reachable; insurer edges had no routes) | `backend/src/security/claimStateMachine.ts` `TRANSITIONS`, `checkTransition()`; `transitionClaim()` did a conditional `UPDATE ... WHERE id = ? AND stage = ?` **then** a separate audit INSERT |
| Audit trail | IMPLEMENTED | `backend/src/security/audit.ts` `writeAuditEvent()`; `backend/migrations/0002_security.sql` `audit_events` with `BEFORE UPDATE` / `BEFORE DELETE` triggers |
| Rate limiting | IMPLEMENTED (approximate, per IP) | `backend/src/index.ts` `/api/*` middleware keyed on `cf-connecting-ip`; `wrangler.toml` `[[ratelimits]]` 100 / 60 s |
| Headers, CORS allowlist, 64 KB body limit, generic errors | IMPLEMENTED | `backend/src/index.ts` |
| Queue message validation | IMPLEMENTED | `backend/src/endpoints/ocr.ts` `processQueueBatch()` zod schema |
...
| Tests | 9 files, 85 tests | `backend/test/*.test.ts`, `backend/vitest.config.mts`, `backend/test/setup.ts`, `backend/test/helpers.ts` |

Phase 0 request pipeline at HEAD `1468897` (real middleware order from `backend/src/index.ts` at that commit):
...
|---|---|---|
| `requireActor` / `resolveActor()` in `actor.ts` | return an `Actor` or `null`; everything downstream depends only on the returned `Actor` | body of `resolveActor()` replaced with JWT verification; `requireActor` kept (adds `WWW-Authenticate`) |
...
| `vitest.config.mts`, `test/setup.ts`, `test/helpers.ts`, `test/env.d.ts` | isolated D1 per test file, migrations + seed applied, `call({ as })` injects an actor | kept; `call({ as })` now mints a real HS256 token instead of setting dev headers, so the six unchanged Phase 0 test files run against JWT auth without edits |
...
|---|---|---|---|---|
| No real authentication (BACKEND-SEC-001; `docs/security/README.md` warning) | NOT IMPLEMENTED | **IMPLEMENTED** | `backend/src/security/actor.ts`: `resolveActor()` parses `Authorization: Bearer <JWT>` (`BEARER_TOKEN` regex), `verify(token, JWT_SECRET, { alg: 'HS256', iss: JWT_ISSUER, aud: JWT_AUDIENCE })`, then `validateClaims()`; `requireActor` returns 401 + `WWW-Authenticate`. `backend/src/types.ts` `Bindings.JWT_SECRET/JWT_ISSUER/JWT_AUDIENCE`. `backend/wrangler.toml` `[vars]` `JWT_ISSUER`, `JWT_AUDIENCE`. `backend/.dev.vars.example` `JWT_SECRET=CHANGE_ME`. `backend/scripts/setup-dev-vars.mjs` (`npm run setup:local`), `backend/scripts/mint-token.mjs` (`npm run token`). Dev header stub removed from `actor.ts`, `types.ts` and the CORS `allowHeaders` in `index.ts`. | `test/auth.test.ts` AUTH-001..014, `test/actor.test.ts` |
...
| Insurer roles not tenant-scoped (BACKEND-SEC-002; README "any ASSESSOR/MANAGER can read any claim"; AUTHZ-15 BLOCKED) | NOT IMPLEMENTED | **IMPLEMENTED** | `backend/migrations/0003_tenants.sql` (`tenants`, `policies.tenant_id`, `claims.tenant_id` with `REFERENCES tenants(id)`, `audit_events.actor_tenant_id`, backfill by seed policy id only, indexes). `backend/src/types.ts` `Actor.tenantId`, `INSURER_ROLES`. `backend/src/security/actor.ts` `validateClaims()` requires `tenant_id` for ASSESSOR/MANAGER, forbids it for CUSTOMER. `backend/src/security/claimAccess.ts` `isTenantInsurer()` and the load -> tenant -> ownership -> mode order in `loadAuthorizedClaim()`, with deny reasons `missing / not_owner / cross_tenant / tenant_unset / draft / role / mode`. `backend/src/endpoints/claims.ts` `POST /initiate` copies `policy.tenant_id` (422 `policy_not_eligible` when NULL); `GET /claims` scoped per owner / per tenant. `backend/seed_sa_data.sql` sets `tenant_id`. | `test/tenant.test.ts` TENANT-001..007, `test/migration.test.ts` |
| Insurer workflow endpoints missing (BACKEND-SEC-003; STATE-10 BLOCKED) | NOT IMPLEMENTED | **IMPLEMENTED** | `backend/src/endpoints/claimsInsurer.ts`: `POST /claims/:claimId/verify`, `/screen`, `/review`, `/request-info`, `/decide`, `/pay`; every route is `insurerOnly` -> `validate('param', claimIdParam)` -> `loadAuthorizedClaim(c, id, 'insurer')` -> `transitionClaim()`. No route writes `claims.stage` directly. Mounted in `index.ts` at `/api/v1/claims`. | `test/tenant.test.ts` STATE-001..006, RBAC-001/002 |
| Strict schemas on every new endpoint (BACKEND-SEC-004) | IMPLEMENTED for existing routes | **IMPLEMENTED** for the new ones | `backend/src/security/validation.ts` `decideSchema` (`outcome: 'Approved' \| 'Rejected'`, `.strict()`), `listQuerySchema` (`limit` 1..50) | `test/tenant.test.ts` "decide rejects invalid or extra fields", TENANT-006 (`?limit=500` -> 400) |
| Per-user rate limits (BACKEND-SEC-012) | PARTIAL (per IP) | **PARTIAL** (per IP + per actor; still approximate; no stricter limits on expensive routes) | `backend/src/index.ts`: second `/api/v1/*` middleware after `requireActor`, key `actor:<role>:<id>`, same `RATE_LIMITER` binding | `test/hardening.test.ts` "rate limits per IP before auth and per actor after auth" |
| Audit write not atomic with the stage update (BACKEND-SEC-013; `AUDIT_SECURITY.md` gaps) | PARTIAL (sequential) | **IMPLEMENTED** | `backend/src/security/audit.ts` `auditStatement(c, event, { onlyIfPreviousChanged: true })` builds `INSERT ... SELECT ... WHERE changes() = 1`; `claimAccess.ts` `transitionClaim()` runs `c.env.DB.batch([update, audit])`, and a 0-row result is audited as `claim.transition_rejected` `stale_state` | `test/audit.test.ts` "a stage-change audit row is written only when the stage change applied (same transaction)", STATE-001 |
| Keep tests green (BACKEND-SEC-014) | IMPLEMENTED locally; CI NOT IMPLEMENTED | **IMPLEMENTED locally** (12 files, 175 passed, 1 todo); CI still **NOT IMPLEMENTED** | `backend/test/*.test.ts` | this report's run |
...
| OCR/AI as untrusted signal (BACKEND-SEC-008) | NOT IMPLEMENTED | **NOT IMPLEMENTED** (unchanged) | `backend/src/endpoints/ocr.ts` unchanged | `test/queue.test.ts` |
| Configuration change audit (BACKEND-SEC-009) | NOT IMPLEMENTED | **NOT IMPLEMENTED** | no config exists | none |
| Stubs not actor-scoped (BACKEND-SEC-010; `mandates/:tenantId/check` unscoped) | stubs | **NOT IMPLEMENTED** (unchanged) | `backend/src/endpoints/identity.ts`, `gateway.ts`, `audit.ts` unchanged; `GET /profile/mandates/:tenantId/check` still ignores `tenantId` | none |
| Queue consumer does not fire under `wrangler dev` (BACKEND-SEC-011) | OPEN | **OPEN, NOT TESTED live** | `processQueueBatch()` unchanged | `test/queue.test.ts` (unit only) |
| Audit triggers droppable by D1 admin (`AUDIT_SECURITY.md`, `THREAT_MODEL.md`) | accepted risk | **OPEN** (unchanged) | `0002_security.sql` triggers untouched; `0003_tenants.sql` adds a column, no new protection | `test/audit.test.ts` "audit rows cannot be updated or deleted" |
| Rate limiting approximate (`BACKEND_INVENTORY.md` section 6) | by design of the binding | **OPEN** (by design) | `wrangler.toml` `[[ratelimits]]` unchanged | `test/hardening.test.ts` |

Phase 1 changes that were not a listed Phase 0 risk but tightened Phase 0 behaviour:

- `backend/src/index.ts`: the hono `requestId()` middleware (which reuses an inbound `X-Request-Id` header if it is well-formed) was replaced by a middleware that always sets `crypto.randomUUID()`, because the id is persisted in `audit_events.request_id`.
- `backend/src/endpoints/claims.ts` `PATCH /:claimId/screening`: the UPDATE now repeats the stage precondition (`AND stage IN ('Draft', 'Info Needed')`) and returns 409 when 0 rows change.
- `backend/src/endpoints/claims.ts` `GET /:claimId/timeline`: a stage counts as completed when the audit trail shows it was reached, so side-path history is not lost.
- `backend/src/security/audit.ts` writes `actor_tenant_id` on every row; `test/audit.test.ts` also asserts no `Bearer` / `eyJ` substrings in audit rows.
- `backend/EasyClaim.postman_collection.json` rewritten for bearer tokens; generated `*.postman_environment.json` files are gitignored (`.gitignore`, `backend/.gitignore`).

Decisions Phase 1 left explicitly open (marked in code):

- `backend/src/types.ts`, `actor.ts` `validateClaims()`: ADMIN may carry or omit `tenant_id` — **DECISION REQUIRED** (platform admin vs tenant admin). Today ADMIN has no claim access either way (`test/tenant.test.ts` TENANT-001b, RBAC-002).
- `backend/src/endpoints/claims.ts` `GET /claims` (insurer branch): `user_id` is a platform-wide customer id and is not returned to tenant staff — **DECISION REQUIRED** (per-tenant claimant reference).
- `backend/src/security/actor.ts` header comment: no revocation list; bounded TTL (`MAX_TOKEN_TTL_SECONDS`) and secret rotation are the compensating controls. External identity provider through `hono/jwt` `verifyWithJwks` is **PLANNED** (documented only, no code path).
...
| Queue consumer under `wrangler dev` (BACKEND-SEC-011) | OPEN, NOT TESTED live | — |
...
| Audit triggers droppable by a D1 admin | OPEN (accepted risk) | operational: restrict account access, periodic export |
...
| Read endpoint for auditors (`/activities/audit-trail` returns `[]`) | NOT IMPLEMENTED | `audit.ts` endpoint |
...
3. **Phase 1 insurer routes depend on the Phase 0 state machine and role gate unchanged.** `backend/src/security/claimStateMachine.ts` and `backend/src/security/rbac.ts` are unchanged from HEAD (not in the modified set). `claimsInsurer.ts` never calls `checkTransition()` to change state; it calls it once in `/pay` only to order the `not_approved` precondition after the role/legality check, and then still goes through `transitionClaim()`.
...
6. **Phase 1 tests depend on the Phase 0 vitest setup.** `vitest.config.mts` (`cloudflareTest` plugin, `readD1Migrations`, seed SQL as a binding), `test/setup.ts` (`applyD1Migrations` + seed per test file) and `test/env.d.ts` are Phase 0 work with small edits. `test/helpers.ts` kept the Phase 0 `call({ as })` signature and the Phase 0 aliases `assessor` / `manager`, which is why `bola.test.ts`, `rbac.test.ts`, `claimLifecycle.test.ts`, `massAssignment.test.ts`, `queue.test.ts` and `stateMachine.test.ts` are unchanged from HEAD and still pass. `migration.test.ts` depends on `TEST_MIGRATIONS` exposing the raw `0003` statements. `test/tenant.test.ts` builds on Phase 0 helpers `createReadyDraft()` (extended with `createSubmittedClaim()`).
...
sequenceDiagram
    ...
    CA-->>IX: {ok:true}
    IX-->>M: 200 {status:'transitioned', from:'Review', to:'Decision', outcome:'Approved'}
```
...
`docs/phase-reports/PHASE_01_PRECHECK.md` section 3 found three discrepancies in the Phase 0 documents. All three are wording; none was a code claim. They are **not** fixed by editing the Phase 0 record retroactively; they are corrected in the Phase 1 documentation update (the same update that removes the dev-header instructions), and this report keeps the history of what was wrong.

| # | Location (Phase 0 record) | Problem | Correct statement | Handling |
|---|---|---|---|---|
| D1 | `docs/security/README.md` line 22 | "Decisions / payouts — NOT IMPLEMENTED — no endpoints", but a read-only `GET /claims/:claimId/decision` existed in Phase 0 (`claims.ts`), and `BACKEND_INVENTORY.md` / the root README already said PARTIAL | Phase 0: "no decision-making or payout endpoints; `GET /decision` is read-only". Phase 1 additionally adds `POST /decide` (outcome only) and `POST /pay` (state transition only) | corrected in the Phase 1 doc update; Phase 0 text left as the historical record |
| D2 | `docs/security/ATTACK_SCENARIOS.md` line 56 | scenario 5 "Evidence tampering" cross-references BACKEND-SEC-008 (OCR/AI); evidence upload is BACKEND-SEC-007 | "See BACKEND-SEC-007" | corrected in the Phase 1 doc update |
| D3 | `docs/security/SECURITY_IMPLEMENTATION_PLAN.md` line 15 | the P0 row label for claim state transitions uses the banned adjective as a control name | "Claim state transition enforcement" | corrected in the Phase 1 doc update |
...
| Check | Result |
|---|---|
| `npx vitest run` in `backend/` (this session) | Test Files 12 passed (12); Tests 175 passed, 1 todo (176) |
| Test files | `actor`, `audit`, `auth`, `bola`, `claimLifecycle`, `hardening`, `massAssignment`, `migration`, `queue`, `rbac`, `stateMachine`, `tenant` |
| The single todo | `test/tenant.test.ts` TENANT-004 evidence resource isolation: BLOCKED, no evidence endpoint |
| Files unchanged from HEAD that Phase 1 depends on | `backend/src/security/claimStateMachine.ts`, `backend/src/security/rbac.ts`, `backend/src/endpoints/ocr.ts`, `identity.ts`, `gateway.ts`, `audit.ts`, `policy.ts`, `backend/migrations/0001_init.sql`, `0002_security.sql`, `backend/test/bola.test.ts`, `rbac.test.ts`, `claimLifecycle.test.ts`, `massAssignment.test.ts`, `queue.test.ts`, `stateMachine.test.ts`, `test/setup.ts`, `test/env.d.ts` |
| `npm run typecheck`, `npm audit` | NOT RE-RUN for this report (Phase 0 values in `PHASE_01_PRECHECK.md` section 1) |
| Live `wrangler dev` checks | NOT TESTED in this report |

---

## Appendix: Phase 1 working-tree change set (from `git status` on `cyber`)

Modified: `.gitignore`, `backend/.dev.vars.example`, `backend/.gitignore`, `backend/EasyClaim.postman_collection.json`, `backend/package.json` (scripts `setup:local`, `secret`, `token`), `backend/seed_sa_data.sql`, `backend/src/endpoints/claims.ts`, `backend/src/index.ts`, `backend/src/security/actor.ts`, `backend/src/security/audit.ts`, `backend/src/security/claimAccess.ts`, `backend/src/security/validation.ts`, `backend/src/types.ts`, `backend/test/actor.test.ts`, `backend/test/audit.test.ts`, `backend/test/hardening.test.ts`, `backend/test/helpers.ts`, `backend/vitest.config.mts`, `backend/wrangler.toml`.

Added: `backend/migrations/0003_tenants.sql`, `backend/scripts/mint-token.mjs`, `backend/scripts/setup-dev-vars.mjs`, `backend/src/endpoints/claimsInsurer.ts`, `backend/test/auth.test.ts`, `backend/test/migration.test.ts`, `backend/test/tenant.test.ts`, `docs/phase-reports/`.
