...
| Signing secret `JWT_SECRET` (Phase 1) | Worker secret binding (`wrangler secret put JWT_SECRET`); `backend/.dev.vars` locally (gitignored) | Critical: whoever holds it can mint a token for any role and any tenant. No rotation procedure (NOT IMPLEMENTED) |
...
| Admin (`ADMIN`; `tenant_id` optional) | No claim access and no transitions today (list → 403, claim routes → 404). Configuration and users are future | Misconfiguration or abuse; platform vs tenant admin scope DECISION REQUIRED |
...
2. **Token issuer → Worker** (Phase 1, NEW): the Worker trusts any token whose HS256 signature verifies with `JWT_SECRET`, whose `iss` / `aud` equal `JWT_ISSUER` / `JWT_AUDIENCE`, and whose claims pass `validateClaims` (`exp` + `iat` required, per-role TTL cap, `sub` / `tenant_id` format, role-dependent `tenant_id` rule). Today the only issuer is `backend/scripts/mint-token.mjs` (local demos); there is no identity provider, user store or revocation list. The boundary *is* the shared secret: it is a Worker secret binding and never in `wrangler.toml`. Missing configuration fails closed (every request 401). PLANNED: external IdP via `hono/jwt` `verifyWithJwks` (asymmetric keys), key rotation, `jti` denylist.
...
| ADMIN scope | `tenant_id` is optional in an ADMIN token and unused; ADMIN has no functions today, so a leaked ADMIN token grants nothing now, but platform vs tenant admin is undecided | DECISION REQUIRED |
...
| 12 | ADMIN token scope (platform vs tenant) | token `tenant_id` (optional, unused) | DECISION REQUIRED (ADMIN currently has no powers, so exposure is nil today) |
