...
| API2 Broken Authentication | Verified identity on every request | There is no identity provider. The dev header stub is spoofable | NOT IMPLEMENTED (fails closed; dev stub only) | `src/security/actor.ts` `resolveActor` | Backend team: replace `resolveActor` with JWT/IdP verification (pinned alg, `exp`, `sub`, `role`) | `test/actor.test.ts` (stub only); JWT tests NOT TESTED | P0 |
...
| API8 Misconfiguration | The dev auth flag never reaches deployment | `ALLOW_DEV_ACTOR_HEADERS=true` in production equals no auth | PARTIAL: it is only in `.dev.vars` (gitignored), not in `wrangler.toml` | `.dev.vars.example` | Add a CI/deploy check that fails if the flag is set outside local | NOT TESTED | P0 |
