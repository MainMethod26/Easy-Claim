...
**This repository contains both the backend API and the Flutter frontend.**

### Project Structure

- **`frontend/`**: The Flutter mobile/web application containing the UI for both Customers and Insurer Staff (Admins, Managers, Assessors).
- **`backend/`**: The backend API powered by Cloudflare Workers (TypeScript) and a D1 database.
- **`quantum/`**: Python module for ML/Quantum logic and screening experiments.
- **`scripts/`**: Maintenance scripts, database migrations, and Python one-off fixes.
...
The server-side state machine (`backend/src/security/claimStateMachine.ts`, unchanged in Phase 1) defines all of these stages, plus a `Draft` pre-state. Since Phase 1 every insurer transition, and every customer transition except Withdrawn, is reachable over HTTP:
...
| Tenant model | IMPLEMENTED (`tenants` table; `policies.tenant_id`, `claims.tenant_id`, `audit_events.actor_tenant_id`; a claim inherits its policy's tenant). Tenant admin functions: NOT IMPLEMENTED | `migrations/0003_tenants.sql`, `backend/test/tenant.test.ts` |
| Claims | PARTIAL (initiate, screening, submit, appeal and list persisted in D1; no evidence) | `backend/src/endpoints/claims.ts` |
| Claim lifecycle | IMPLEMENTED state machine; over HTTP PARTIAL (all insurer transitions plus the customer transitions Draft → Submitted, Info Needed → Screening and Decision → Appeal; Withdrawn and Expired have no endpoint or job) | `backend/src/security/claimStateMachine.ts`, `backend/src/security/claimAccess.ts`, `backend/src/endpoints/claimsInsurer.ts` |
...
| OCR/text extraction | NOT IMPLEMENTED (stub endpoint + queue consumer that logs) | `backend/src/endpoints/ocr.ts` |
...
| Audit logging | IMPLEMENTED (append-only `audit_events` incl. acting tenant and server-generated request id; a stage change and its audit row are written in one D1 batch) | `backend/src/security/audit.ts`, `migrations/0002_security.sql`, `migrations/0003_tenants.sql` |
...
| Tests | TESTED/PASSED (17 Worker test files, 264 passed; 27 Python tests for the quantum pipeline) | `backend/test/`, `quantum/tests/` |
...
backend/test/                   vitest security, tenant, auth and lifecycle tests
